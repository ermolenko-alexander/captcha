...
class mydataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while len(image_name) < captcha_setting.MAX_CAPTCHA:
            image_name += ' '
        label = ohe.encode(image_name) # 为了方便，在生成图片的时候，图片文件的命名格式 "4个数字或者数字_时间戳.PNG", 4个字母或者即是图片的验证码的值，字母大写,同时对该值做 one-hot 处理
        return image, label
